...
const addGameMessage = function(message: string) {
...
type card = [score: number, image: string];
...
const give_card = function(index: number) {
    const card = cards[index];
    return card
}


const calculate_score = function (usercard: card[]) {
    let score = 0;
    usercard.forEach((each_card: card) => {
        score += each_card[0];
    })
    if (score > 21) {
        return 0
...
const check_result = function (score_fisrt: number, score_second =50) {
...
hit!.addEventListener("click", (event: Event)=>{
        event!.preventDefault();
        gameForm!.hidden = true;
...
stand!.addEventListener("click", (event: Event)=>{
        event!.preventDefault();
        gameForm!.hidden=true;
        deal();
        addGameMessage(choose_winner());
...
double!.addEventListener("click", (event: Event)=> {
        event!.preventDefault();
        gameForm!.hidden=true;
        const user_index = create_random_index();
        const user_draw = give_card(user_index);
